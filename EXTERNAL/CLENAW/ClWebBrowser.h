...
class ClWebBrowser : public CWnd, IBGTransfer
{
protected:
	DECLARE_DYNCREATE(ClWebBrowser)

public:
	ClWebBrowser(void) { m_GpsData.m_bValid= 0, m_dwCTS = 0xFFFFFF; m_pTextLatitude=NULL; m_pTextLongitude = NULL; m_pTextSpeed = NULL; m_pTextAltitude = NULL; m_eUnitOfSpeed = UNIT_KM_P_H; m_bIsMapShow = FALSE; }
...
public:
	...
	VOID SetUnitOfSpeed(ENUM_UNIT_OF_SPEED eUnitOfSpeed) 
	{
		m_eUnitOfSpeed  = eUnitOfSpeed;		
		Synchronize(m_dwCTS, TRUE); //redraw
	}

	VOID SetWindowRect(CRect * pCRect = NULL)
	{
		if(pCRect)
			m_rectClient = *pCRect;
		else
	...
	public:
			...
			HRESULT					ExcuteScript		( const CString & FuncName, const CStringArray & strParamArr,
															const std::vector<VARENUM>&  vtArr , VARIANT * vaResult=NULL);
			CString					ExecuteScript2		(const CString& strFunction);
			void					Synchronize			(BOOL internet_use,  unsigned long dwCTS , BOOL bRedraw = FALSE);
			void 				drawPoly					( void );
			void					SetSensorData		( CTextData* pSensorData);
			VOID 				SetUiText(CStatic* pTextLatitude, CStatic* pTextLongitude, CStatic* pTextSpeed = NULL, CStatic* pTextAltitude = NULL) ;
			
			VOID 				DrawText(double dLatitude = 0, double dLongitude = 0, double dSpeed = 0) ;
			
			BOOL					IsInternetConnected	( VOID ) { return this->m_bIsNavigate; };
			VOID					SetInternetConnected( BOOL bNavigate = TRUE ) { this->m_bIsNavigate = bNavigate; };

			BOOL					IsGPSData			( VOID ) { if( !this->m_pSensorData ) return FALSE; return !this->m_pSensorData->m_GPSList.empty(); };
			
	virtual VOID					TransferBackground	( CDC* pDC ){ UNREFERENCED_PARAMETER(pDC); };

public:
			double					m_fLat, m_fLng, m_fSpeed;
			CGPSData				m_GpsData;
protected:
			...
			unsigned long 				m_dwCTS;
			CStatic* 					m_pTextLatitude; //UI
			CStatic* 					m_pTextLongitude; //UI
			CStatic* 					m_pTextAltitude; //UI
			CStatic*					m_pTextSpeed;
