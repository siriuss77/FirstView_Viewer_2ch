...
typedef void (*pfnFileMergeCB)(int nTotal, int nCurrent);
...
public:
	...
	RECT	m_rtWindowSize3;
	RECT	m_rtWindowSize4;
...
public:
	...
	HRESULT	GetBritnessValue(int nVideoCh, int *nEffectNum, int *iEffectValue);
	BOOL GetCaptureToDib(int nVideoCh, CBitmap& bitmapImage);
	...
	void Initialize(int nNumberOfWindows, HWND hWnd1, HWND hWnd2=NULL, HWND hWnd3=NULL, HWND hWnd4=NULL);	// 플레이어 초기화
	...
	BOOL OpenFile( const WCHAR* szFilename, void* hNxfsDev, int iNxfsAssignedFd);	//	파일 열기	
	...
	HRESULT SetBrightness(int nVideoCh, int iValue);
	HRESULT	SetBrightnessValue(int nVideoCh, int nEffectNum, int iEffectValue);
	HRESULT SetFlip(int nVideoCh, bool bType);
	HRESULT SetMirror(int nVideoCh, bool bType);
	...
	BOOL FileMerge(WCHAR* pwszDstFullPath, WCHAR** pwszSrcFullPathArray, int nSrcFiles, int nVideoCh, pfnFileMergeCB pFunc, void* hNxfsDev, int iNxfsAssignedFd, BOOL bDec);
	BOOL FileCopy(WCHAR* pwszDstFullPath, WCHAR* pwszSrcFullPath, pfnFileMergeCB pFunc, void* hNxfsDev, int iNxfsAssignedFd, BOOL bDec);
