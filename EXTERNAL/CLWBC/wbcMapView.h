...
class CWBCMapView : public CWnd
{
protected:
	DECLARE_DYNCREATE(CWBCMapView)
...
public:
	...
	//
			...
			void					Synchronize			( unsigned long dwCTS );
			void					setTextData         ( CWBCTextData* pSensorData );
            ...
            BOOL                    IsValidWindow       ( VOID ) { RECT rect; this->GetWindowRect(&rect); if( (rect.right - rect.left) == 0 ) return FALSE; return TRUE; }
			BOOL					IsGPSData			( VOID ) { if( !this->m_pTextData ) return FALSE; return !this->m_pTextData->m_GPSList.empty(); };

	virtual VOID					TransferBackground	( CDC* pDC ){ UNREFERENCED_PARAMETER(pDC); };

protected:

			CWBCTextData*	        m_pTextData;
			BOOL					m_bIsNavigate;
};
//================================================================================================= E. CLASS
