// wbcSlider.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(CWBCSlider, CStatic)
    //{{AFX_MSG_MAP(CWBCSlider)
    ON_WM_ERASEBKGND()
    ON_WM_PAINT()
    ON_WM_LBUTTONDOWN()
    ON_WM_MOUSEMOVE()
...
/////////////////////////////////////////////////////////////////////////////
// CWBCSlider message handlers

BOOL CWBCSlider::OnEraseBkgnd(CDC* pDC) 
{
...
// 컨트롤을 그린다
//
void CWBCSlider::OnPaint() 
{
    CPaintDC dcOrigin(this);

    // "Flicker Free Drawing In MFC" by Keith Rule
    CWBCMemDC dc( &dcOrigin, &m_rect, m_bTransparentChannel );
...
//      [IN]    nMax
//              최대값.
//      [IN]    bRedraw//              TRUE이면 컨트롤을 다시 그리고

//              FALSE이면 최대값만 변경합니다.
//
void CWBCSlider::SetRangeMax(int nMax, BOOL bRedraw)
{
    m_nMax = nMax;
    if( bRedraw )
        Invalidate();
...
//              FALSE이면 최대값만 변경합니다.
//
void CWBCSlider::SetRangeMin(int nMin, BOOL bRedraw)
{
    m_nMin = nMin;
    if( bRedraw )
        Invalidate();
...
//                FALSE이면 최대값만 변경합니다.
//
void CWBCSlider::SetRange(int nMin, int nMax, BOOL bRedraw)
{
...
//                새로운 위치 값.
//
void CWBCSlider::SetPos(int nPos)
{
    m_nPos = nPos;
...
//        이전의 페이지 값.
//
int CWBCSlider::SetPageSize(int nSize)
{
    int nRet = m_nPage;
...
//
void CWBCSlider::SetMargin(int nLeft, int nTop, int nRight, int nBottom )
{
    SetMarginLeft( nLeft );
    SetMarginTop( nTop );
...
//                FALSE는 동작을 억제합니다.
//
void CWBCSlider::Enable(BOOL bEnable)
{
    m_bEnable = bEnable;
...
//                FALSE는 상태값만을 바꿉니다.
//
void CWBCSlider::DrawFocusRect(BOOL bDraw, BOOL bRedraw)
{
...
//
BOOL CWBCSlider::SetBitmapChannel(
...
//        성공했으면 TRUE 를 그렇지 않으면FALSE를 리턴합니다
//
BOOL CWBCSlider::SetBitmapThumb(
...
// 드래그가 시작될 때
//
void CWBCSlider::OnLButtonDown(UINT nFlags, CPoint point) 
{
    if( !m_bEnable )
        return;
...
// 드래그 중일 때
//
void CWBCSlider::OnMouseMove(UINT nFlags, CPoint point) 
{
    ...
    if( Pos2Pixel(m_nPos) != nPixel ) {
        ...
        ::PostMessage(
            GetParent()->GetSafeHwnd(), WM_WBCSLIDER_MOVING,
            GetDlgCtrlID(), m_nPos );
    }
...
// 드래그가 끝났을 때
//
void CWBCSlider::OnLButtonUp(UINT nFlags, CPoint point) 
{
    if( !m_bEnable )
        return;
    ...
    ::PostMessage(
        GetParent()->GetSafeHwnd(),    WM_WBCSLIDER_MOVED,
        GetDlgCtrlID(), m_nPos );
...
//
int CWBCSlider::Pos2Pixel(int nPos)
{
    if( m_bVertical ) {
...
//
int CWBCSlider::Pixel2Pos(int nPixel)
{
    if( m_bVertical ) {
...
//
void CWBCSlider::CopyBackground(
...
// 비트맵을 지정한 영역에 그린다
//
void CWBCSlider::RestoreBackground(
    ...
    COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY)
{
    ...
    hdcDst.StretchBlt(0,0,bm.bmWidth, bm.bmHeight, &hdcSrc,0,0,bm.bmWidth, bm.bmHeight,SRCCOPY);
...
// By Raja Segar
//
void CWBCSlider::DrawTransparentBitmap(
    CDC *pDC, int xStart, int yStart, int wWidth, int wHeight,
...
// 키보드의 입력을 처리한다
//
void CWBCSlider::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    if( !m_bEnable )
        return;
...
// 컨트롤이 포커스를 잃었을 때
//
void CWBCSlider::OnKillFocus(CWnd* pNewWnd) 
{
...
// 컨트롤이 포커스를 얻었을 때
//
void CWBCSlider::OnSetFocus(CWnd* pOldWnd) 
{
